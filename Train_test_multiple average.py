...
num_experiments = 2
train_ratio = 0.3  # 30% for training, 70% unseen test
...
for exp_idx in range(num_experiments):
    ...
    for fold_idx, (cv_train_idx, cv_val_idx) in enumerate(cv.split(train_data)):
        ...
        for step in range(steps):
            ...
            # --- Method B ---
            entropy_plot_B, unitaries_B = safe_qnnTraining_methodB(qnnArch, unitaries_B, cv_train, lda=2, ep=0.1, rounds=training_intervals)
            ...
            # --- Method A ---
            entropy_plot_A, unitaries_A = safe_qnnTraining_methodA(qnnArch, unitaries_A, cv_train, lda=2, ep=0.1, rounds=training_intervals)
...
for metric, ylabel in zip(
    ...
    plot_metric(metric, ylabel, f"{metric.replace('_',' ').upper()} Comparison", f"{metric}_comparison.png")

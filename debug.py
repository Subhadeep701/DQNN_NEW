...
num_experiments = 1
train_ratio = 0.75  # 30% for training, 70% unseen test
...
# -----------------------------
# ============================================================
# MAIN EXPERIMENT LOOP WITH HEAVY DEBUGGING
# ============================================================
for exp_idx in range(num_experiments):
    print(f"\n===== Experiment {exp_idx+1}/{num_experiments} =====")
    np.random.seed(1 + exp_idx)
    data = randomNetwork(qnnArch, num_samples)
    all_data = data[2]
    initial_unitaries = data[1]

    print(f"[DEBUG] Initial unitary outer list id: {id(initial_unitaries)}")
    for l, layer in enumerate(initial_unitaries):
        for j, u in enumerate(layer):
            print(f"[DEBUG] id(initial_unitaries[{l}][{j}]) = {id(u)}")
    ...
    for fold_idx, (cv_train_idx, cv_val_idx) in enumerate(cv.split(train_data)):
        ...
        # independent copies of unitaries
        unitaries_A = unitariesCopy(initial_unitaries)
        unitaries_B = unitariesCopy(initial_unitaries)

        # debug IDs for mutation detection
        print("[DEBUG] After unitariesCopy IDs:")
        for l,(ua,ub) in enumerate(zip(unitaries_A,unitaries_B)):
            for j,(a,b) in enumerate(zip(ua,ub)):
                print(f" layer {l} unit {j}  idA:{id(a)} idB:{id(b)}")

        # Stepwise metrics
        train_entropy_A, cv_entropy_A, train_fid_A, cv_fid_A = [], [], [], []
        train_entropy_B, cv_entropy_B, train_fid_B, cv_fid_B = [], [], [], []
        test_entropy_A, test_fid_A, test_entropy_B, test_fid_B = [], [], [], []

        # to track gradients/changes
        prev_unitaries_A = unitariesCopy(unitaries_A)
        prev_unitaries_B = unitariesCopy(unitaries_B)
        prev_train_ent_A = prev_train_ent_B = None

        for step in range(steps):
            print(f"\n[STEP] Exp {exp_idx+1}|Fold {fold_idx+1}|Step {step+1}/{steps}")

            # --- Method B ---
            entropy_plot_B, unitaries_B = safe_qnnTraining_methodB(
                qnnArch, unitaries_B, cv_train, lda=2, ep=0.09, rounds=training_intervals
            )

            # Evaluate
            tr_ent_B = evaluate_qnn(qnnArch, unitaries_B, cv_train)
            cv_ent_B = evaluate_qnn(qnnArch, unitaries_B, cv_val)
            tr_fid_B = evaluate_fidelity(qnnArch, unitaries_B, cv_train)
            cv_fid_B = evaluate_fidelity(qnnArch, unitaries_B, cv_val)

            # Debug gradient exploration
            if prev_train_ent_B is not None:
                delta_ent = tr_ent_B - prev_train_ent_B
                print(f"[DEBUG][B] ΔTrainEntropy = {delta_ent:+.4e}")
            prev_train_ent_B = tr_ent_B

            # Debug unitary change
            frob_norm_B = 0.0
            for l,(prevL,newL) in enumerate(zip(prev_unitaries_B, unitaries_B)):
                for j,(p,n) in enumerate(zip(prevL,newL)):
                    diff = (p - n).norm()
                    frob_norm_B += diff**2
                    print(f"[DEBUG][B] Layer {l} Unit {j} ||ΔU||={diff:.4e}")
            frob_norm_B = np.sqrt(frob_norm_B)
            print(f"[DEBUG][B] Total Frobenius change = {frob_norm_B:.4e}")
            prev_unitaries_B = unitariesCopy(unitaries_B)

            # Save metrics
            train_entropy_B.append(tr_ent_B)
            cv_entropy_B.append(cv_ent_B)
            train_fid_B.append(tr_fid_B)
            cv_fid_B.append(cv_fid_B)
            ...
            entropy_plot_A, unitaries_A = safe_qnnTraining_methodA(
                qnnArch, unitaries_A, cv_train, lda=2, ep=0.09, rounds=training_intervals
            )

            tr_ent_A = evaluate_qnn(qnnArch, unitaries_A, cv_train)
            cv_ent_A = evaluate_qnn(qnnArch, unitaries_A, cv_val)
            tr_fid_A = evaluate_fidelity(qnnArch, unitaries_A, cv_train)
            cv_fid_A = evaluate_fidelity(qnnArch, unitaries_A, cv_val)

            if prev_train_ent_A is not None:
                delta_ent = tr_ent_A - prev_train_ent_A
                print(f"[DEBUG][A] ΔTrainEntropy = {delta_ent:+.4e}")
            prev_train_ent_A = tr_ent_A

            frob_norm_A = 0.0
            for l,(prevL,newL) in enumerate(zip(prev_unitaries_A, unitaries_A)):
                for j,(p,n) in enumerate(zip(prevL,newL)):
                    diff = (p - n).norm()
                    frob_norm_A += diff**2
                    print(f"[DEBUG][A] Layer {l} Unit {j} ||ΔU||={diff:.4e}")
            frob_norm_A = np.sqrt(frob_norm_A)
            print(f"[DEBUG][A] Total Frobenius change = {frob_norm_A:.4e}")
            prev_unitaries_A = unitariesCopy(unitaries_A)

            train_entropy_A.append(tr_ent_A)
            cv_entropy_A.append(cv_ent_A)
            train_fid_A.append(tr_fid_A)
            cv_fid_A.append(cv_fid_A)

...
def safe_qnnTraining_methodA(qnnArch, unitaries, trainingData, lda, ep, rounds):
    unitaries_copy = unitariesCopy(unitaries)
    training_copy = [copy.deepcopy(d) for d in trainingData]
    return qnnTraining_en(qnnArch, unitaries_copy, training_copy, lda, ep, rounds, alert=0)

def safe_qnnTraining_methodB(qnnArch, unitaries, trainingData, lda, ep, rounds):
    unitaries_copy = unitariesCopy(unitaries)
    training_copy = [copy.deepcopy(d) for d in trainingData]
    return qnnTraining(qnnArch, unitaries_copy, training_copy, lda, ep, rounds, alert=0)
...
# -----------------------------
results = {
    "A": {"train": [], "train_fidelity": [], "test_entropy": [], "test_fidelity": []},
    "B": {"train": [], "train_fidelity": [], "test_entropy": [], "test_fidelity": []}
}
...
# -----------------------------
for fold_idx, (train_idx, test_idx) in enumerate(cv.split(all_data)):
    print(f"\n=== Fold {fold_idx+1}/{k_folds} ===")
    training_data = [copy.deepcopy(all_data[i]) for i in train_idx]
    testing_data  = [copy.deepcopy(all_data[i]) for i in test_idx]

    unitaries_A = unitariesCopy(initial_unitaries)
    unitaries_B = unitariesCopy(initial_unitaries)

    # Per-fold results
    train_A, train_fid_A, test_entropy_A, test_fid_A = [], [], [], []
    train_B, train_fid_B, test_entropy_B, test_fid_B = [], [], [], []

    for step in range(steps):
        print(f"Step {step+1}/{steps}")

        # --- Method B (Existing/Fidelity-based) ---
        entropy_plot_B, unitaries_B = safe_qnnTraining_methodB(
            qnnArch, unitaries_B, training_data, lda=4, ep=0.1, rounds=training_intervals)
        train_B.append(entropy_plot_B[2][-1])
        train_fid_B.append(evaluate_fidelity(qnnArch, unitaries_B, training_data))
        test_entropy_B.append(evaluate_qnn(qnnArch, unitaries_B, testing_data))
        test_fid_B.append(evaluate_fidelity(qnnArch, unitaries_B, testing_data))

        # --- Method A (Entropy-based) ---
        entropy_plot_A, unitaries_A = safe_qnnTraining_methodA(
            qnnArch, unitaries_A, training_data, lda=4, ep=0.1, rounds=training_intervals)
        train_A.append(entropy_plot_A[2][-1])
        train_fid_A.append(evaluate_fidelity(qnnArch, unitaries_A, training_data))
        test_entropy_A.append(evaluate_qnn(qnnArch, unitaries_A, testing_data))
        test_fid_A.append(evaluate_fidelity(qnnArch, unitaries_A, testing_data))

    # Store fold results
    results["A"]["train"].append(train_A)
    results["A"]["train_fidelity"].append(train_fid_A)
    results["A"]["test_entropy"].append(test_entropy_A)
    results["A"]["test_fidelity"].append(test_fid_A)

    results["B"]["train"].append(train_B)
    results["B"]["train_fidelity"].append(train_fid_B)
    results["B"]["test_entropy"].append(test_entropy_B)
    results["B"]["test_fidelity"].append(test_fid_B)

# -----------------------------
# Compute mean + CI per method
# -----------------------------
summary = {}
for method in ["A","B"]:
    summary[method] = {}
    for metric in results[method]:
        mean_vals, ci_vals = mean_ci(results[method][metric], confidence_level)
        summary[method][metric] = (mean_vals, ci_vals)

# -----------------------------
# Plotting: Training Set - Entropy
# -----------------------------
plt.figure(figsize=(6,5))
for method, style, color in [("A","o-","blue"), ("B","s--","red")]:
    mean_vals, ci_vals = mean_ci(results[method]["train"], confidence_level)
    label = "Proposed Method (Entropy)" if method=="A" else "Existing Method (Fidelity)"
...
plt.title("Training Set: Entropy Cost Comparison")
plt.grid(True, linestyle='--', alpha=0.6)
plt.legend()
plt.tight_layout()
plt.savefig("Train_Entropy_Comparison.png", dpi=300)
plt.show()

# -----------------------------
# Plotting: Training Set - Fidelity
# -----------------------------
plt.figure(figsize=(6,5))
for method, style, color in [("A","o-","blue"), ("B","s--","red")]:
    mean_vals, ci_vals = mean_ci(results[method]["train_fidelity"], confidence_level)
    label = "Proposed Method (Entropy)" if method=="A" else "Existing Method (Fidelity)"
    plt.errorbar(x_axis_values, mean_vals, yerr=ci_vals, fmt=style, color=color, capsize=4, label=label)
plt.xlabel("Training Rounds")
plt.ylabel("Fidelity")
plt.title("Training Set: Fidelity Comparison")
plt.grid(True, linestyle='--', alpha=0.6)
plt.legend()
plt.tight_layout()
plt.savefig("Train_Fidelity_Comparison.png", dpi=300)
plt.show()

# -----------------------------
# Plotting: Test Set - Entropy
# -----------------------------
plt.figure(figsize=(6,5))
for method, style, color in [("A","o-","blue"), ("B","s--","red")]:
    mean_vals, ci_vals = summary[method]["test_entropy"]
    label = "Proposed Method (Entropy)" if method=="A" else "Existing Method (Fidelity)"
    plt.errorbar(x_axis_values, mean_vals, yerr=ci_vals, fmt=style, color=color, capsize=4, label=label)
plt.xlabel("Training Rounds")
plt.ylabel("Entropy Cost")
plt.title("Test Set: Entropy Cost Comparison")
plt.grid(True, linestyle='--', alpha=0.6)
plt.legend()
plt.tight_layout()
plt.savefig("Test_Entropy_Comparison.png", dpi=300)
plt.show()

# -----------------------------
# Plotting: Test Set - Fidelity
# -----------------------------
plt.figure(figsize=(6,5))
for method, style, color in [("A","o-","blue"), ("B","s--","red")]:
    mean_vals, ci_vals = summary[method]["test_fidelity"]
    label = "Proposed Method (Entropy)" if method=="A" else "Existing Method (Fidelity)"
    plt.errorbar(x_axis_values, mean_vals, yerr=ci_vals, fmt=style, color=color, capsize=4, label=label)
plt.xlabel("Training Rounds")
plt.ylabel("Fidelity")
plt.title("Test Set: Fidelity Comparison")
plt.grid(True, linestyle='--', alpha=0.6)
plt.legend()
plt.tight_layout()
